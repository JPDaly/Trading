...
def voting():
	...
	for n_sectors,sector in enumerate(companies['GICS industry group'].drop_duplicates()):
		...
		print("Working on sector: {0}/{1}".format(n_sectors+1,len(companies['GICS industry group'].drop_duplicates())), end='\r')
		...
		sector_df = sector_df.merge(find_patterns(companies['ASX code'].values),on='asx code')
